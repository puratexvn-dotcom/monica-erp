import { Factory, AlertTriangle, CircleAlert, CheckCircle2, Gauge } from 'lucide-react';
...
// ============================================================================
// FEED ADAPTER — CHUYỀN MAY · Blueprint hợp nhất *(Board Directive 05/08/2026)*
//
...
//
// 🔑 Nhờ vậy khối MOS **⛔ không biết gì về Chuyền may** — nó ⛔ không có bảng tra
 ...
 *
 * Chuyền may ⛔ **không** đo mức khẩn bằng ngày — nó đo bằng **mức nghiêm trọng**. Quy
...
export function mayFeed(
  ...
  const tasks: MosTask[] = vao.viec.map((v) => ({
    ...
    // ⚠️ Chuyền may ⛔ chưa có câu phụ riêng cho từng việc. Để **chuỗi rỗng**, ⛔ KHÔNG
  ...
  const kpis: MosKpi[] = vao.kpi.map((k) => ({
    ...
    icon: k.id === 'hieu-suat' ? Gauge : Factory,
    // ④ HÀNH ĐỘNG — `P39` `P40`. Nhãn nói **đi đâu**, ⛔ không nói *"xem thêm"*.
    goLabel: t(k.hanhDongKey ?? 'workspace.drillDown'),
    ...(k.href ? { onGo: nhay(k.href) } : {}),
  }));

  // ⚠️ Chuyền may ⛔ **chưa có** nguồn cảnh báo riêng — mọi phán đoán đang nằm ở
  // hộp thư việc. Trả **mảng rỗng**, ⛔ KHÔNG nhân bản việc sang thành cảnh
  // báo cho ô đỡ trống: một cảnh báo trùng một việc là **hai lần nhắc cùng một
  // chuyện**, và người dùng sẽ học cách bỏ qua cả hai.
