...
export default function InquiryFormDialog({
  ...
  useEffect(() => {
    if (open) reset({ ...defaults, received_date: vnToday() });
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    if (!res.ok) {
      ...
      toast.error('Không tạo được yêu cầu báo giá', { description: res.message });
      return;
    }
    toast.success('Đã tạo yêu cầu báo giá', { description: res.message });
    onClose();
    await onCreated();
  });

  return (
    <Modal open={open} title="Nhận yêu cầu báo giá mới" onClose={onClose} wide>
      <form onSubmit={onSubmit} noValidate>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button type="submit" className={btnPrimary} disabled={formState.isSubmitting || customers.length === 0}>
